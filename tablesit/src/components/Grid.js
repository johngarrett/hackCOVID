...
export class Grid extends React.Component {
    static propTypes = {
        ...
        desk_x: PropTypes.number,
        desk_y: PropTypes.number,
    ...
    constructor(props) {
        ...
        if (this.props.grid.length > 0) {
            ...
            grid = new Array(props.height);
            for (let i = 0; i < props.height; i++) {
                grid = new Array(props.length);
            }
        }
    ...
    render() {
        return <h1> GRIDSDSD </h1>;
    }

    seating(social_distance) {
    /**
    ...
    */
        ...
        let searched_desks = [];
        let good_desks = [];
        let unsearched_desks = [{x:0,y:0}];

        while (unsearched_desks.length > 0) {
            let {dx, dy} = unsearched_desks.pop();
            ...
            for (let i = 0; i < 2; i++) {
                for (let j = 0; j < 2; j++) {
                    next_desks.append({
                        x: Math.min(dx + 1, this.props.height - 1),
                        y: Math.min(dy + 1, this.props.length - 1)
                    });
                }
            }

            for (let d in next_desks) {
                if (searched_desks.includes(d)) {
                    continue;
                } else {
                    unsearched_desks.append(d);
                }

                let desk_obj = this.state.grid[d[0]][d[1]];
                if (!desk_obj || desk_obj.is_obstruction) {
                    continue;
                }

                if (this.desk_is_properly_spaced(d, good_desks, social_distance, {x: this.props.desk_x, y: this.props.desk_y})) {
                    good_desks.append(d);
                }
            }
    ...
    desk_is_properly_spaced(d, good_desks, social_distance, {desk_x, desk_y}) {
        return false;
    }
}
/*
